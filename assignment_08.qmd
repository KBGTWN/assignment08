...
grep("^v", colnames(votes))#checking if the columns you want to select actually start with "v" ... they do.

############ I keep getting errors with the ^ code so I tried to adjust with as.numeric below but I keep getting more issues. 
#Error: Error in colnames(x) : argument "x" is missing, with no default
###Unsuccessful troubleshooting below:
...
if (anyNA(votes_numeric)) {
  # Remove rows with missing values
  votes_numeric <- na.omit(votes_numeric)
}

votes_numeric <- select_if(votes_103, is.numeric)
votes_numeric <- as.data.frame(votes_numeric)


# Identify and remove duplicate rows
dups <- duplicated(votes_numeric)
votes_numeric <- votes_numeric[!dups, ]
...
max_clusters <- nrow(votes_numeric)
...
#####Can't get sum of squares ^ to work properly
...
# Load the data
votes <- read.csv("votes_103.csv")

# Remove the party column
votes_numeric <- votes[, -1]
